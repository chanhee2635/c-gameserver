...
> Windows **IOCP** 비동기 네트워크와 **액터 모델 JobQueue**(Zone당 단일 스레드 직렬화)로 구현한 MMO형 게임 서버.
> 단일 PC에서 **최대 3,000 동시접속(CCU)** 을 패킷 유실 0으로 수용하며, 부하 테스트로 병목을 측정·개선한 과정을 기록했습니다.
...
**② 현재 빌드 한계 측정** *(sceneCount=8 · HEARTBEAT 500ms · 0→3,000 CCU ramp, 2026-06-07)*

| 동시접속 | CPU (프로세스) | 메모리 | 평균 틱 지연 | 합산 처리량 (In+Out) | 패킷 유실 |
|---|---|---|---|---|---|
| 2,000 CCU | 11 % | 63 MB | 12 ms | ~50k pkt/s | **0** |
| **3,000 CCU** (피크) | **~20 %** | 75 MB | **12 ms** | **~80k pkt/s** (out ~4.5 MB/s) | **0** |

> 4종 프로세스를 한 PC에 올린 보수적 환경에서 **3,000 CCU를 패킷 유실 0 · 수신 백로그 0 · DB 실패 0**으로 수용.
> CPU는 동접에 거의 선형(3,000명에서도 프로세스 ~20%)이고 틱 지연은 전 구간 평탄(AvgLag 12ms) — 직전 빌드의 1.6초 틱 스파이크를 **send-lane 병렬화·하트비트 500ms·이동 변화 게이팅**으로 제거. → [측정 리포트](docs/loadtest-report.md)

![CCU 대비 CPU·틱 지연](docs/loadtest-perf.svg)

> CPU는 동접에 거의 선형(3,000명 ~20%), 평균 틱 지연은 전 구간 12ms로 평탄, 메모리는 50→75MB로 평탄.
...
flowchart TD
    ...
    C -- "HTTP / REST" --> L["LoginWebServer :5245<br/>ASP.NET Core · BCrypt 인증·토큰 발급"]
    ...
    subgraph GS ["GameServer 내부 (액터 모델 JobQueue 계층)"]
...
**데이터 흐름:** 로그인(웹 BCrypt 인증·토큰 발급) → Redis 토큰 저장 → GateServer 대기열 입장(100ms 배치 admission) → GameServer 접속 시 Redis 토큰 검증(1회용) → Zone 기반 AOI 동기화 → 변경분만 Write-Back으로 MySQL 저장.
...
- **액터 모델 JobQueue** — Zone/Scene을 **한 번에 한 스레드만 실행**하도록 직렬화 → 컨텐츠 로직을 락 없이 작성(데드락 원천 차단). 큐 자체는 경량 뮤텍스로 보호 · [`ServerCore/JobQueue.h`](ServerCore/JobQueue.h)
...
- **SendBuffer 풀링 & 모아쏘기** — 패킷마다 `WSASend` 대신 **lock-free SList 청크 풀**(TLS)에서 할당, 프레임별 일괄 flush · [`ServerCore/SendBuffer.cpp`](ServerCore/SendBuffer.cpp)
- **Write-Back DB 영속화** — 이동마다 UPDATE 금지, 메모리 누적 후 레벨업/로그아웃 시점에만 비동기 저장 (ODBC 커넥션 풀)
- **대기열 입장 제어(Admission)** — GateServer가 Redis 점유율을 게이트로 **100ms마다 배치 입장**시켜(스폰 평탄화) 폭주 방지 · [`GateServer/QueueManager.cpp`](GateServer/QueueManager.cpp)
...
- **몬스터 AI** — **격자(GridMap) 기반 A\* 경로탐색**(Unity NavMesh를 walkability 격자로 추출·로드), Idle/Moving/Return/Attack/Dead FSM, cross-scene 타겟 탐색 · [`GameServer/GridMap.cpp`](GameServer/GridMap.cpp)
- **이동 검증(anti-cheat) · DoS 가드** — 서버가 격자 walkability(직전→목적지 세그먼트)·속도 게이트로 클라 무결성 독립 검증·롤백 보정, 세션당 수신 **50pkt/s 초과 시 차단** + 채팅 쿨다운(월드 5s/일반 0.5s)
...
| 문제 | 원인 | 해결 | 결과 |
|---|---|---|---|
| 2,000명에서 한 틱 최대 1.6초 지연 | 브로드캐스트가 scene 단일 Job으로 워커 1개 독점 | zone별 send-lane 8개로 전송 offload·병렬화 | MaxLag 1.6s→정상 ~13ms, **3,000명까지 확장** ([분석](docs/tick-performance-analysis.md)) |
...
| 구분 | 기술 |
|---|---|
| Server | C++17, Windows IOCP, Google Protobuf, MySQL(ODBC), Redis(hiredis) |
| Web | ASP.NET Core, Entity Framework, JWT |
| Client | Unity (C#), protobuf-net |
| 패턴 | IOCP Proactor, 액터 모델 JobQueue, Lock-Free 풀(SList)+TLS, Zone AOI, Write-Back |
| 부하테스트 | DummyClient(3,000 동접, 16그룹×8워커) + ramp 드라이버·로그 파서(로컬 하네스) |
...
4. 설정: `GameServer/server.json.example`→`server.json`, `LoginWebServer/appsettings.example.json`→`appsettings.json` 복사 후 DB 비밀번호 설정
5. 사전 구동: Redis(:6379), MySQL(:3306, DB명 portfolio)
6. 실행 순서: LoginWebServer → GameServer → (GateServer) → Client/DummyClient
```
...
주요 흐름·명세는 [`docs/api.md`](docs/api.md) · [`docs/database.md`](docs/database.md), AOI 최적화는 [`AOI_MOVEMENT_PIPELINE.md`](AOI_MOVEMENT_PIPELINE.md) 참고.
